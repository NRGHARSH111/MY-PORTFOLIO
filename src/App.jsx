...
const SOCIAL_ICONS = {
  ...
  linkedin: 'M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z',
...
function App() {
  ...
  useEffect(() => {
    const lenis = new Lenis({
      duration: 1.8, // Slower, luxurious feel
      easing: (t) => Math.min(1, 1.001 - Math.pow(2, -10 * t)),
      orientation: 'vertical',
      gestureOrientation: 'vertical',
      smoothWheel: true,
      wheelMultiplier: 0.5, // Reduced speed for smoother feel
      touchMultiplier: 1.0,
      infinite: false,
    })
  ...
  return (
    <AnimatedGridBackground effect={0}>
      <main ref={mainRef} className="relative min-h-screen px-6">
        ...
        <button
          onClick={scrollToTop}
          className={`fixed bottom-6 right-6 z-50 p-4 rounded-full bg-gradient-to-r from-cyan-500/30 to-blue-500/30 backdrop-blur-md border-2 border-cyan-400/50 text-cyan-300 hover:from-cyan-500/50 hover:to-blue-500/50 hover:border-cyan-400/80 hover:text-white hover:scale-110 transition-all duration-500 shadow-[0_0_20px_rgba(6,182,212,0.3)] hover:shadow-[0_0_30px_rgba(6,182,212,0.6)] group ${showScrollTop ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8 pointer-events-none'}`}
